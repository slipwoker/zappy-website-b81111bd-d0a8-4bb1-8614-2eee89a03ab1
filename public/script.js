...
// Cookie Consent Initialization (multi-language) /* __ccConfigBarV3 */
...
(function() {
  ...
  "guiOptions": {
    ...
    "preferencesModal": {
      ...
      // Inject an X dismiss button into the slim bar when the consent modal
      // shows. Clicking it accepts ONLY the necessary category (i.e. rejects
      // all non-necessary cookies) — dismissal without explicit consent is
      // not consent under GDPR/ePrivacy, so this is the correct mapping.
      // Functions can't survive JSON.stringify, so we attach the callback
      // after the config object is parsed at runtime.
      __ccConfig.onModalShow = function(payload) {
        try {
          if (payload && payload.modalName !== 'consentModal') return;
          var modalEl = document.querySelector('#cc-main #cm.cm--bar') ||
                        document.querySelector('#cc--main #cm.cm--bar');
          if (!modalEl || modalEl.querySelector('.cm__close-x')) return;
          var lang = (cc.getUserPreferences && cc.getUserPreferences().language) ||
                     __ccConfig.language.default || 'en';
          var labels = __ccCloseLabels || {};
          var label = labels[lang] || labels.en || 'Close';
          var btn = document.createElement('button');
          btn.type = 'button';
          btn.className = 'cm__close-x';
          btn.setAttribute('aria-label', label);
          btn.innerHTML = '\u00D7';
          btn.addEventListener('click', function(ev) {
            ev.preventDefault();
            ev.stopPropagation();
            try { cc.acceptCategory([]); } catch (_) {}
            try { cc.hide(); } catch (_) {}
          });
          modalEl.appendChild(btn);
        } catch (_) {
          // Defensive — never let the close-button injection break the bar.
        }
      };
